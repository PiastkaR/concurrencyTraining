...
public class MainApplication {
    ...
    private static class ThreadSafeQueue {
        private Queue<MatricesPair> queue = new LinkedList<>();//LL is not thread safe!
        private boolean isEmpty = true;
        private boolean isTerminate = false;
        private static final int CAPACITY = 5;//for backpressure

        public synchronized void add(MatricesPair matricesPair) {//synchronized used for keeping it atomic for notify()
        ...
        public synchronized void terminate() {
            isTerminate = true;
            notifyAll();//to decouple producers and consumers
        }
    }
